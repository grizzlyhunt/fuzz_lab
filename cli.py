...
def triage_crash(args):
    ...
    compare both reports' SUMMARY lines and print whether they agree. Requires
    build_fuzzer to have been run first.
    """
    extra_args = [f"-max_total_time={args.time}"]
...
def dedupe_crashes(args):
    ...
    already there instead of re-running), and deletes any whose ASan SUMMARY line
    matches crash_path's exactly -- along with their '.log'/'.min' derivatives, since
    they are redundant reports of the same bug. Requires build_fuzzer to have been run
    first.
    """
